"""Twelve Data market-data source (forex + gold) — live with API key(s).

Configure one or more keys (free tier: 800 credits/day, 8 req/min each):
  TWELVEDATA_KEY=primary
  TWELVEDATA_KEYS=key2,key3,key4,key5
  TWELVEDATA_KEY_2=...  TWELVEDATA_KEY_3=...  etc.

When a key hits daily credits or rate limits, the pool rotates automatically.
"""
...
def available() -> bool:
    return get_pool().status()["active"] > 0 or get_pool().status()["configured"] > 0
...
def fetch_ohlcv(pair: str, tf: str, outputsize: int = 5000) -> pd.DataFrame:
    ...
    if not pool._keys:
        raise RuntimeError(
            "TWELVEDATA_KEY not set — add TWELVEDATA_KEY and optional TWELVEDATA_KEYS on Railway"
        )
